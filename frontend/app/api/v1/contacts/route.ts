...
/**
 * GET /api/v1/contacts?email=&external_id=&limit=&cursor=
 *
 * Lists the org's contacts (donors), newest first, keyset-paginated.
...
export const GET = withApiV1(async (req, ctx) => {
  ...
  if (error) {
    console.error("[api/v1/contacts] query error:", error.message)
    return apiErrorResponse(500, "internal_error", "Failed to load contacts.")
  }
