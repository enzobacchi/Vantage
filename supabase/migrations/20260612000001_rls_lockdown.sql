...
--    cross-org with the public anon key. Only the service role needs these.
revoke execute on function public.match_donors(public.vector, double precision, integer, uuid) from anon, authenticated;
revoke execute on function public.user_org_membership(uuid) from anon, authenticated;
...
alter function public.match_donors(public.vector, double precision, integer, uuid) set search_path = '';
alter function public.report_retention_rate(uuid, integer) set search_path = '';
alter function public.report_acquisition_rate(uuid, integer) set search_path = '';
alter function public.report_recapture(uuid, integer) set search_path = '';
alter function public.report_new_leads_by_source(uuid, integer) set search_path = '';
