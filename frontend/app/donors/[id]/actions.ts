...
export async function getDonorProfile(id: string): Promise<DonorProfileResult> {
  ...
  const { data: donor, error: donorError } = await supabase
    .from("donors")
    .select(
      "id,org_id,display_name,first_name,last_name,email,phone,billing_address,city,state,zip,mailing_address,mailing_city,mailing_state,mailing_zip,total_lifetime_value,last_donation_date,notes,donor_type,acquisition_source"
    )
    .eq("id", id)
    .eq("org_id", org.orgId)
    .maybeSingle()
